...
public static class ModifiedUtf8
{
    ...
    private static void GetStringCommon(ReadOnlySpan<byte> bytes, int stringLength, ref char destination)
    {
        ...
        {
            if (BitConverter.IsLittleEndian && Avx2.IsSupported && bytes.Length >= 32)
            {
    ...
    private static void GetStringAsciiAvx2(ReadOnlySpan<byte> bytes, ref char destination)
    {
        ...
        while (Unsafe.IsAddressLessThan(ref source, ref sourceEnd))
        {
            ...
            Unsafe.WriteUnaligned(ref target, Sse2.UnpackLow(lower, Vector128<byte>.Zero).AsInt16());
            target = ref Unsafe.Add(ref target, 16);

            Unsafe.WriteUnaligned(ref target, Sse2.UnpackHigh(lower, Vector128<byte>.Zero).AsInt16());
            target = ref Unsafe.Add(ref target, 16);

            Unsafe.WriteUnaligned(ref target, Sse2.UnpackLow(upper, Vector128<byte>.Zero).AsInt16());
            target = ref Unsafe.Add(ref target, 16);

            Unsafe.WriteUnaligned(ref target, Sse2.UnpackHigh(upper, Vector128<byte>.Zero).AsInt16());
            target = ref Unsafe.Add(ref target, 16);
